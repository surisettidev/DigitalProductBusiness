// ─── main-v3.js — product card renderer for v3 design ─────────
(function () {
  'use strict';
  ...
  const FEATURES = {
    ...
    'pricing-guide':  ['Value-based pricing calc', 'Proposal templates',      'Scope creep scripts'],
    'tax-guide':      ['GST registration guide',   '44ADA tax savings',       'Invoice template'],
    'client-gen':     ['Cold outreach sequences',  'Lead tracker sheet',      'Discovery call script'],
    'business-os':    ['Notion CEO dashboard',     'Weekly ops playbook',     'Burnout prevention OS'],
  };
  ...
  if (grid && cfg.products) {
    grid.innerHTML = ORDER.map((slug, idx) => {
      ...
      return `
<article class="product-card${featured ? ' featured' : ''}" data-reveal data-delay="${idx + 1}">
  ${badgeHtml}
  ...
  <div class="card-footer">
    ...
    <button class="buy-btn" data-slug="${slug}">
      Buy now <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12,5 19,12 12,19"/></svg>
    </button>
  </div>
